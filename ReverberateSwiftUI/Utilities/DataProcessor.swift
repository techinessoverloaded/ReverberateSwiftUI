...
class DataProcessor {
    ...
    static func getAlbum(named albumName: String) -> Album?
    {
        return DataManager.availableAlbums.first(where: { $0.name! == albumName })
    }

    static func getAlbumThat(containsSong songName: String) -> Album?
    {
        return DataManager.availableAlbums.first(where: { $0.songs!.contains(where: { $0.title! == songName }) })
    }

    static func getArtist(named artistName: String) -> Artist?
    {
        return DataManager.availableArtists.first(where: { $0.name! == artistName })
    }

    static func getSong(named songName: String) -> Song?
    {
        return DataManager.availableSongs.first(where: { $0.title! == songName })
    }
...
//    }
    ...
    static func getSongsThatSatisfy(theQuery query: String, songSource: [Song]? = nil) -> [SongSearchResult]?
    {
        var result: [SongSearchResult] = []
        let songs = songSource != nil ? songSource! : DataManager.availableSongs
...
//    }
    ...
    static func getAlbumsThatSatisfy(theQuery query: String, albumSource: [Album]? = nil) -> [AlbumSearchResult]?
    {
        var result: [AlbumSearchResult] = []
        let albums = albumSource != nil ? albumSource! : DataManager.availableAlbums
        let keyword = query.lowercased()
        for album in albums
        {
...
//    static func getPlaylistsThatSatisfy(theQuery query: String) -> [PlaylistSearchResult]?
//    {
