...
struct DataManager
{
    private(set) static var availableSongs: [Song] = []
    
    private(set) static var availableAlbums: [Album] = []
    
    private(set) static var availableArtists: [Artist] = []
    
    private static var songCacheKey = "songCache"
    
    private static var albumCacheKey = "albumCache"
    
    private static var artistCacheKey = "artistCache"
    
    private static func getSongs() -> [Song]
    {
    ...
    static func makeSongsAndAlbumsReady(onCompletion completionHandler: ((DispatchTime) -> Void)? = nil)
    {
    ...
    private static func getUrlOfDataInCacheDirectory(dataFileName: String) -> URL
    {
        return FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0].appendingPathComponent(dataFileName)
    }
    
    private static func persistDataToCache(fileName: String)
    {
    ...
    private static func tryRetrieveDataFromCache<T>(typeOfData type: T.Type, fileName: String) -> Array<T>?
    {
        let dataUrl = getUrlOfDataInCacheDirectory(dataFileName: fileName)
...
//    static func persistRecentlyPlayedItems(songName: String, albumName: String)
//    {
...
//    }
//
//    static func retrieveRecentlyPlayedItems()
//    {
...
//    }
//
//    static func clearRecentlyPlayedItems()
//    {
