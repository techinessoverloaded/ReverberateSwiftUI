...
struct SongMetadataExtractor
{
    ...
    public static func extractSongMetadata(withFileName fileName: String, ofLanguage language: Language, ofGenre genre: MusicGenre) -> Song?
    {
    ...
    @available(iOS 16.0, *)
    static func extractSongMetadataAsync(withFileName fileName: String, ofLanguage language: Language, ofGenre genre: MusicGenre) async -> Song?
    {
        guard let url = Bundle.main.url(forResource: fileName.nameWithoutExtension, withExtension: fileName.extension) as NSURL? else
        {
            return nil
        ...
        song.genre = genre
        song.language = language
        guard let metadata = try? await avUrlAsset.load(.metadata) else {
            return nil
        }
        for metadataItem in metadata
        {
            ...
            if key == .commonKeyAlbumName
            {
                guard let albumName = try? await metadataItem.load(.stringValue)?.trimmedCopy else {
                    continue
                }
                song.albumName = albumName
                print(song.albumName ?? "")
            ...
            if key == .commonKeyTitle
            {
                guard let title = try? await metadataItem.load(.stringValue)?.trimmedCopy else {
                    continue
                }
                song.title = title
                print(song.title ?? "")
            ...
            if key == .commonKeyArtist
            {
                guard let artistNames = try? await metadataItem.load(.stringValue)?.trimmedCopy else {
                    continue
                }
                if artistNames.contains(",")
                {
            ...
            if key == .commonKeyArtwork
            {
                guard let data = try? await metadataItem.load(.dataValue), let cover = UIImage(data: data) else {
                    continue
                }
                song.coverArt = cover
                print(cover)
    ...
    @available(iOS 16.0, *)
    static func extractMultipleSongsAsync(withFileNames songFileNames: [String], ofLanguageGenreAndCount languageGenreAndCount: [(language: Language, genre: MusicGenre, count: Int)]) async -> [Song]
    {
        var songs: [Song?] = []
        var index = 0
        var offset = 0
